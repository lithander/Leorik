...
namespace Leorik.Search
{
    public class IterativeSearch : ISearch
    {
        ...
        private int EvaluateRootBNS(int depth)
        {
            ...
            // Set initial bounds based on the allowed score range.
            int lowerBound = MIN_ALPHA;
            int upperBound = MAX_BETA;
            Move bestMove = default;
            int bestScore = lowerBound;

            // Iteratively narrow the range. (The stopping condition here is when the interval is narrow enough.)
            while (upperBound - lowerBound > 1 && !_killSwitch.Get())
            {
                int test = (lowerBound + upperBound) / 2;
                int successCount = 0;
                Move candidateMove = default;
                int candidateScore = lowerBound;

                // Evaluate each root move with a null-window search [test, test+1].
                for (int i = 0; i < RootMoves.Length; i++)
                {
                    Move move = RootMoves[i];
                    if (!next.Play(root, ref move))
                        continue;
                    int score = EvaluateNext(0, depth, test, test + 1, moveGen);
                    if (score >= test + 1)
                    {
                        successCount++;
                        candidateMove = move;
                ...
                if (successCount == 1)
                {
                    // Exactly one move exceeds the test threshold: we choose it.
                ...
                {
                    // No move succeeded – all moves score below the test value.
                    upperBound = test;
                }
                else
                {
                    // More than one move succeeded – raise the lower bound.
                    lowerBound = test + 1;
                }
            }

            // If no unique candidate was found, do a full window search among all root moves.
            if (bestMove == default)
            {
                ...
                for (int i = 0; i < RootMoves.Length; i++)
                {
                    ...
                    if (!next.Play(root, ref move))
                        continue;
                    int score = EvaluateNext(0, depth, lowerBound, upperBound, moveGen);
                    if (score > bestScore)
                    {
                        bestScore = score;
                        bestMove = move;
            ...
            // Promote best move to the front.
            for (int i = 0; i < RootMoves.Length; i++)
            {
